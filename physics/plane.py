...
class Plane:
    def __init__(self, Cd_0, Em, e_w, S, b, Cl_max, Lam, tc_max, W_struct, W_fuel, W_engines, W_payload, cj, T_a_sl, n_struct, atmosphere):
        # design parameters
    ...
    def d_takeoff(self, rolling_coeff, wing_height):
        ...
        speed = self.speed_takeoff()
        # calculate average value of all retarding forces (use V=0.7V_lo)
        Cd_i = self.ground_effect(wing_height) * self.Cd_i(self.Cl_max)
        drag = self.drag(self.Cd(Cd_i), speed*0.7)
        lift = Plane.convert_speed_to_q(self.atm_density, speed*0.7) * self.S * self.Cl_max
        friction = rolling_coeff * (self.W_0 - lift)
        retarding = drag + friction
